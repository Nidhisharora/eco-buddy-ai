...
class CircularDependencyError(ValueError):
    """Raised when a circular dependency is detected in the milestone graph."""
    pass


class MilestoneDependencyError(ValueError):
    """Raised when an operation violates a dependency constraint."""
    pass
...
class RoadmapMilestone:
    """Represents a single milestone in a sustainability roadmap."""
...
class SustainabilityRoadmap:
    """Represents a user's overarching sustainability roadmap."""
...
def init_roadmap_db(db_name: str = DB_NAME) -> None:
    """Initializes the database schema for the roadmap system."""
...
def _validate_no_circular_dependencies(dependencies: List[Tuple[int, int]]) -> None:
    """
    Validates that a given set of dependencies forms a Directed Acyclic Graph (DAG).
    Raises CircularDependencyError if a cycle is detected.
    
    Args:
        dependencies: A list of (milestone_id, depends_on_id) tuples.
    """
...
def _topological_sort(milestones: List[RoadmapMilestone], dependencies: List[Tuple[int, int]]) -> List[RoadmapMilestone]:
    """
    Sorts milestones in topological order based on dependencies.
    """
    ...
    # Reconstruct milestones list in sorted order
...
def get_roadmap_graph_data(roadmap_id: int) -> Dict[str, Any]:
    """
    Retrieves the roadmap milestones and dependencies structured for UI graph visualization.
    """
...
def create_roadmap(user_id: int, title: str, target_date: Optional[datetime] = None) -> SustainabilityRoadmap:
    """Creates a new empty roadmap for a user."""
    with database_connection(DB_NAME) as conn:
        cursor = conn.cursor()
...
def get_roadmap(roadmap_id: int) -> Optional[SustainabilityRoadmap]:
    """Retrieves a roadmap and its milestones by ID."""
    with database_connection(DB_NAME) as conn:
        cursor = conn.cursor()
...
def get_active_roadmap_for_user(user_id: int) -> Optional[SustainabilityRoadmap]:
    """Retrieves the active roadmap for a given user."""
    with database_connection(DB_NAME) as conn:
        cursor = conn.cursor()
...
def get_milestone(milestone_id: int) -> Optional[RoadmapMilestone]:
    """Retrieves a milestone by ID along with its dependencies."""
    with database_connection(DB_NAME) as conn:
        cursor = conn.cursor()
...
def get_milestones_for_roadmap(roadmap_id: int) -> List[RoadmapMilestone]:
    """Retrieves all milestones for a roadmap."""
    with database_connection(DB_NAME) as conn:
        cursor = conn.cursor()
...
def add_dependency(milestone_id: int, depends_on_id: int, dep_type: str = DEP_BLOCKING) -> None:
    ...
    with database_connection(DB_NAME) as conn:
        ...
        # Check if they are in the same roadmap
        ms1 = get_milestone(milestone_id)
        ms2 = get_milestone(depends_on_id)
        
        if not ms1 or not ms2:
            raise ValueError("Invalid milestone IDs.")
            
        if ms1.roadmap_id != ms2.roadmap_id:
            raise ValueError("Cannot add dependencies between milestones in different roadmaps.")
            
        # Get all existing dependencies for the roadmap to check for cycles
        cursor.execute("""
...
def update_milestone_status(milestone_id: int, status: str) -> None:
    """Updates a milestone's status and potentially cascades updates to dependencies."""
    with database_connection(DB_NAME) as conn:
        cursor = conn.cursor()
        now = datetime.now()
    ...
    # Re-evaluate all milestones in the roadmap
...
def update_milestone_progress(milestone_id: int, progress: float) -> None:
    ...
    if new_val >= ms.target_value:
        new_status = STATUS_COMPLETED
    elif new_val > 0 and ms.status == STATUS_ACTIONABLE:
...
def evaluate_roadmap_statuses(roadmap_id: int) -> None:
    """
    Scans all milestones in a roadmap. Unlocks actionable ones, locks blocked ones.
    Handles alternative pathways logic (if one path is taken, skip others).
    """
    milestones = get_milestones_for_roadmap(roadmap_id)
    ...
    # Track which alternative groups have a completed/in-progress item
    alt_groups_active = {}
    for m in milestones:
    ...
    with database_connection(DB_NAME) as conn:
        ...
        for m in sorted_ms:
            # Skip evaluation for completed, skipped, or missed
            ...
            if m.alternative_group_id and m.status in [STATUS_LOCKED, STATUS_ACTIONABLE]:
                if alt_groups_active.get(m.alternative_group_id, False):
                    # Another alternative was chosen, skip this one
            ...
            # Only update if changed, and don't overwrite IN_PROGRESS
...
def update_roadmap_overall_progress(roadmap_id: int) -> None:
    """Recalculates and updates the overall progress of the roadmap."""
    milestones = get_milestones_for_roadmap(roadmap_id)
    if not milestones:
        return
...
def estimate_completion_dates(roadmap_id: int) -> None:
    """
    Estimates completion dates for all milestones in the roadmap
    based on difficulty, dependencies, and user pacing.
    """
    milestones = get_milestones_for_roadmap(roadmap_id)
    ...
    # Calculate historical pace (simplified: could integrate with actual user history)
    # 1 difficulty point = ~3 days of effort
    ...
    with database_connection(DB_NAME) as conn:
        ...
        for m in sorted_ms:
            ...
            # Find the latest completion date of all blocking dependencies
            latest_dep_date = now
            for dep in m.dependencies:
                if dep["dependency_type"] == DEP_BLOCKING:
            ...
            # Add estimated effort duration
            effort_days = m.difficulty * days_per_difficulty_point
            
            # If currently in progress, reduce estimated effort based on progress
...
def detect_missed_milestones(roadmap_id: int) -> List[RoadmapMilestone]:
    """
    Scans for milestones where the target date has passed and grace period expired.
...
def reschedule_missed_milestones(roadmap_id: int, shift_days: int = 14) -> None:
    """
    Shifts the target dates of all missed milestones forward, setting them back to
    ACTIONABLE/LOCKED depending on dependencies.
    """
    milestones = get_milestones_for_roadmap(roadmap_id)
    now = datetime.now()
...
def generate_personalized_roadmap(user_id: int) -> SustainabilityRoadmap:
    """
    Generates a personalized roadmap for a user by analyzing their goals,
    past assessments, and habits.
    """
    # 1. Fetch user data (Simulated here, would use goals.py, database.py etc.)
    with database_connection(DB_NAME) as conn:
        cursor = conn.cursor()
        ...
        # Get baseline assessment data to personalize (just stubbing the query structure)
        ...
        priorities = {CAT_TRANSPORT: 0, CAT_ENERGY: 0, CAT_DIET: 0}
        if assessment:
            transport_type, distance, electricity, diet, flights = assessment
            if electricity and electricity > 300:
                priorities[CAT_ENERGY] += 5
            if transport_type in ['Car', 'SUV'] and distance > 50:
                priorities[CAT_TRANSPORT] += 5
            if diet in ['Meat Heavy', 'Average']:
                priorities[CAT_DIET] += 3
        else:
            # Defaults
            priorities = {CAT_TRANSPORT: 3, CAT_ENERGY: 3, CAT_DIET: 3, CAT_WASTE: 3}
    ...
    # 3. Generate Milestones based on priorities
    # Phase 1: Awareness & Easy Wins
    ...
    # Phase 2: Structural Changes (Depends on Phase 1)
    if priorities[CAT_ENERGY] >= 3:
        ...
        # Alternative Paths: Solar vs Green Grid
