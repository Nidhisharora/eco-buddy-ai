...
def save_voice_log(user_id: int, parsed: dict[str, Any]) -> tuple[bool, str]:
    ...
    try:
        if action == "transport":
            ...
            from src.community.gamification import check_badge_eligibility
        ...
        if action == "water":
            from src.core.database import save_water_assessment
        ...
        if action == "recycling":
            ...
            from src.environment.waste import calculate_waste_footprint
        ...
        if action == "mission":
            from src.utils.sustainability_missions import complete_mission
