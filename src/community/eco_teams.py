...
def _get_user_latest_waste(user_id: int) -> dict[str, Any] | None:
    from src.core.database import get_waste_assessments
...
def _get_user_water_logging(user_id: int) -> dict[str, Any] | None:
    from src.core.database import get_water_assessments
    ...
    try:
        ...
        if new_status == "completed":
            ...
            for (uid,) in contributors:
                from src.core.database import award_xp
            ...
            # Award team badge
            from src.environment.ai_waste_sorter import WASTE_CATEGORIES  # import to avoid circular
            conn.execute(
